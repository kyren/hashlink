...
#[derive(Debug)]
pub struct LruCacheVisitor<K, V> {
    marker: PhantomData<LruCache<K, V>>,
}

impl<K, V> LruCacheVisitor<K, V> {
    fn new() -> Self {
        LruCacheVisitor {
            marker: PhantomData,
        }
...
impl<K, V> Default for LruCacheVisitor<K, V> {
    fn default() -> Self {
        Self::new()
    }
}
    ...
    V: Deserialize<'de>,
{
    ...
    #[inline]
    fn visit_map<M: MapAccess<'de>>(self, mut map: M) -> Result<Self::Value, M::Error> {
        let mut values = LruCache::new(map.size_hint().unwrap_or(usize::MAX));
...
{
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        deserializer.deserialize_map(LruCacheVisitor::default())
    }
}
